...
describe('pathnameOf', () => {
	it('strips origin, query and hash', () => {
		expect(pathnameOf('https://cloud.example/custom_apps/thematiq/css/tokens/x.css?v=abc-36#top'))
			.toBe('/custom_apps/thematiq/css/tokens/x.css')
	})

	it('leaves a root-relative path alone apart from the query', () => {
		expect(pathnameOf('/custom_apps/thematiq/css/tokens/x.css?v=1.1.12'))
			.toBe('/custom_apps/thematiq/css/tokens/x.css')
	})
...
describe('layerKey', () => {
	it('keys a file layer by pathname so two cache-busters are the same file', () => {
		const server = { kind: 'file', href: '/custom_apps/thematiq/css/tokens/a.css?v=d98319d8-36' }
		const manifest = { kind: 'file', href: '/custom_apps/thematiq/css/tokens/a.css?v=1.1.12' }
	...
	it('keys an inline layer by its element id', () => {
		expect(layerKey({ kind: 'inline', id: 'nldesign-logo-url', css: ':root{}' }))
...
describe('diffLayers', () => {
	const designSystem = [
		{ kind: 'file', layer: 'design-system', href: '/a/css/systems/nldesign/theme.css?v=1' },
		{ kind: 'file', layer: 'design-system', href: '/a/css/systems/nldesign/overrides.css?v=1' },
	]
	const amsterdam = designSystem.concat([
		{ kind: 'file', layer: 'tokens', href: '/a/css/tokens/amsterdam.css?v=1' },
		{ kind: 'inline', layer: 'logo-url', id: 'nldesign-logo-url', css: ':root{--nldesign-logo-url:url(/a/img/logos/amsterdam.svg)}' },
	])
	const zwolle = designSystem.concat([
		{ kind: 'file', layer: 'tokens', href: '/a/css/tokens/zwolle.css?v=2' },
		{ kind: 'inline', layer: 'logo-url', id: 'nldesign-logo-url', css: ':root{--nldesign-logo-url:none}' },
	])

	it('reports the set-specific file as removed and added, the shared ones as unchanged', () => {
		const diff = diffLayers(amsterdam, zwolle)
		expect(diff.removed.map(layerKey)).toEqual(['file:/a/css/tokens/amsterdam.css'])
	...
	it('ignores cache-busting differences', () => {
		const rebusted = amsterdam.map((layer) =>
			layer.kind === 'file' ? { ...layer, href: layer.href.replace(/v=\d/, 'v=9') } : layer,
		)
	...
	it('accepts null or undefined lists', () => {
		expect(diffLayers(null, undefined)).toEqual({ removed: [], added: [], unchanged: [] })
	})
})

describe('bumpVersion', () => {
	it('replaces an existing v= whatever its value', () => {
		expect(bumpVersion('/apps/theming/theme/default.css?plain=1&v=11cfaf9b', '42'))
			.toBe('/apps/theming/theme/default.css?plain=1&v=42')
		expect(bumpVersion('/x.css?v=1.1.12-unstable.20260831125624', 'now'))
			.toBe('/x.css?v=now')
	})
